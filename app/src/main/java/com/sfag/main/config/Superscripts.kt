...
/**
 * Utility for displaying text with superscripts. Converts ASCII caret notation (^n, ^2) to Unicode
 * superscripts for UI display.
 */
object Superscripts {
    ...
    /**
     * Converts caret notation to Unicode superscripts for display. If a character has no
     * superscript equivalent, keeps the ^x format as fallback.
     *
     * Examples: "a^n" -> "aⁿ" "a^n b^n" -> "aⁿ bⁿ" "a^2" -> "a²" "ww^R" -> "wwᴿ" "a^x" -> "a^x"
     * (fallback - no superscript for x)
     */
