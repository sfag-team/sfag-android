...
private val CONNECTION_RADIUS = NODE_RADIUS + NODE_OUTLINE_WIDTH / 2f
...
fun toScreenCenter(pos: Offset, density: Density): Offset = with(density) {
...
fun Machine.computePaths(
    ...
    density: Density
): List<TransitionRenderData?> {
    val r = CONNECTION_RADIUS
    val halfSpread = r / sqrt(2f)
    val result = arrayOfNulls<TransitionRenderData>(transitions.size)
    ...
    for ((idx, transition) in transitions.withIndex()) {
        ...
        if (transition.startState == transition.endState) {
            // Self-loop: determine above/below based on connected states
            val stateY = startPos.y
            val connectedYs = transitions.mapNotNull { t ->
                when {
                    t.startState == transition.startState && t.endState != transition.startState ->
                        positions[t.endState]?.y
                    t.endState == transition.startState && t.startState != transition.startState ->
                        positions[t.startState]?.y
                    else -> null
                }
            }
            val above = connectedYs.count { it < stateY }
            val below = connectedYs.count { it > stateY }
            // More above → loop below; more below (or equal) → loop above
            ...
            val controlPoint = Offset(
                midX + CURVATURE * perpX * chordLen,
                midY + CURVATURE * perpY * chordLen
            )
    ...
    for (geom in geoms) {
        ...
        if (geom.isSelfLoop) {
            ...
            result[idx] = TransitionRenderData(
                bodyPath = bodyPath,
                headPath = headPath,
